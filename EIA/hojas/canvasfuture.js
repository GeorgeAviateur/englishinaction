...
function envia(rotelo) {
	...
	if (rotelo == 1) {
		...
		setTimeout(function () {
			window.location = "videos/cinema/cinemaf.html?" + rotelo;
		}, 1000);
	} else
	if (rotelo == 2) {
		if (cadVariables == '11' || cadVariables == '21' || cadVariables == '31' || cadVariables == '41') {
		...
		setTimeout(function () {
			window.location = "videos/bar/barf.html?" + rotelo;
		}, 1000);
	} else
	if (rotelo == 3) {
		if (cadVariables == '11' || cadVariables == '21' || cadVariables == '31' || cadVariables == '41') {
		...
		setTimeout(function () {
			window.location = "videos/gym/gymf.html?" + rotelo;
		}, 1000);
	} else
	if (rotelo == 4) {
		if (cadVariables == '11' || cadVariables == '21' || cadVariables == '31' || cadVariables == '41') {
		...
		setTimeout(function () {
			window.location = "videos/food/foodf.html?" + rotelo;
		}, 1000);
	} else
	if (rotelo == 5) {
		if (cadVariables == '11' || cadVariables == '21' || cadVariables == '31' || cadVariables == '41') window.location = "videos/know/knowfuture.html?" + cadVariables;
		if (cadVariables != '11' || cadVariables != '21' || cadVariables != '31' || cadVariables != '41') window.location = "videos/know/knowfuture.html?5";
	}
	if (rotelo == 100) {
		window.location = "actividades/final_test/ruletina/ruleta.html";
	}
}
...
function left(){
	...
	} else if (x >= myWidth - (ciclaWidth + 38) && xback < 500) {
		xback += 2;
		xtit -= 10;
		xcredit -= 1;
...
function whichButton(event) {
	...
	var ciclaWidth = $('#ciclista').innerWidth();
	var ciclaHeight = $('#ciclista').innerHeight();
	if (event.keyCode == '39' && x <= myWidth - (ciclaWidth + 38)) {
		x = x + 15;
		$('#ciclista').css('left', x);
		$('#tres').css('display', 'none');
		$('#cuatro').css('display', 'none');
	...
	if (event.keyCode == '39' && x >= myWidth - (ciclaWidth + 38) && xback < 500) {
		xback += 2;
		xtit -= 10;
		xcredit -= 1;
		$("#canvasContainer").css('background-position', xback + '% 100%');
	...
	if (event.keyCode == '37' && x <= 3 && xback > 0) {
		xback -= 2;
		xtit += 10;
		xcredit += 1;
		$("#canvasContainer").css('background-position', '' + xback + '% 100%');
	...
	if (event.keyCode == '37' && lock == 1) { //si la tecla presionada es direccional izquierdo
		posicionC = 1;
		if (x > 0) {
			x = x - 15; //mueve la nave 5 pixeles a la izquierda
			lienzo();
