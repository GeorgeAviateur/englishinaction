...
function readme() {
	...
	temp = setTimeout(function(){$('#r1').css('opacity', '1');changehand(2);}, 16000);
...
function goAll(){
    ...
    $('#instruccion').css('z-index', '-3');
    $('#objetivos').css('z-index', '-3');
    $('#txt').css('z-index', '-3');
    $('#r1').css('z-index', '11');
    $('a').css('pointer-events', 'auto');
    $('#startActi').css('z-index', '8');
    ins();
    ins();
    $('#alumbrains').removeClass('tossing');
    $('#alumbraobj').removeClass('tossing');
    var frack = document.getElementById('canvas');
    if(frack)
...
function readme(time){
	...
	$('#menuForAll a').css('pointer-events', 'auto');
    ...
    temp = setTimeout(function(){
		...
		$('#alumbrains').removeClass('bigEntrance');
...
tiempito = 15000;
	...
	temp = setTimeout(function(){$('#instruccion').css('visibility', 'hidden');}, 16000);
	temp = setTimeout(function(){
		$('#r1').css('opacity', '1');
		$('#blanco').css('opacity', '1');
		changehand(2);
...
function readme3(){
	...
	document.getElementById('instruyeme').play();
	temp = setTimeout(function(){document.getElementById('hablame').play();}, 45500);
	temp = setTimeout(function(){$('#r1').css('display', 'inline-block');
	$('#r1').css('width', '98%');
	...
	temp = setTimeout(function(){$('.sig').css('display', 'inline-block');
...
$('#instruccion').css('display', 'inline-block');
	...
	document.getElementById('instruyeme').play();
	temp = setTimeout(function(){document.getElementById('hablame').play();}, 25000);
	temp = setTimeout(function(){ins();ins();$('#r1').css('opacity', '1');}, 25000);
	temp = setTimeout(function(){$('.sig').css('display', 'inline-block');
	document.getElementById('conversation').play();}, 25500);
}
...
function readme5(){
	...
	temp = setTimeout(function(){$('#r1').css('opacity', '1');changehand(2);ins();}, 10000);
...
function readme5(time){
	...
	temp = setTimeout(function(){$('#r1').css('opacity', '1');changehand(2);}, time);
...
if(sobre>3){
	...
	if(cuentabien>(sobre/2+1))
		document.getElementById("retrowell").play();
		else
		if(cuentabien<(sobre/2+1))
		document.getElementById("retrobad").play();
		else
		if(cuentabien == (sobre/2+1))
		document.getElementById("retrowell").play();
